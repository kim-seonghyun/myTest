...
public class UserRepositoryImpl implements UserRepository {
    ...
    public Optional<User> findByUserIdAndUserPassword(String userId, String userPassword) {
        /*todo#3-1 회원의 아이디와 비밀번호를 이용해서 조회하는 코드 입니다.(로그인)
          해당 코드는 SQL Injection이 발생합니다. SQL Injection이 발생하지 않도록 수정하세요. -> PreparedStatement로 SQL Injection 방지!
         */
        ...
        try (PreparedStatement psmt = connection.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                User user = new User(
                        ...
                        Objects.nonNull(rs.getTimestamp("created_at")) ? rs.getTimestamp("created_at").toLocalDateTime()
                                : null,
